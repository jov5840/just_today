$(function(){
          ...
          $('.mainTodo .todo').empty();
               todos.forEach(item => {
                  ...
                  if(toDay == today ){
                     ...
                     <li>
                        <div class="todolist-content">
                           <div class="todolist-left">
                              ...
                              <label class="check">
                                    <input type="checkbox" name="check" class="chk_${today}">
                                    <i class="material-icons checkbox">${item.todocheck}</i>
                              </label>
                           </div>
                     ...
                     `);
                     // 추가 해봅시다.
                     $('.mainTodo .todo').find('input.chk_'+today).click(function() {
                        $('.mainTodo .todo').find('input.chk_'+today).prop("checked",$(this).prop('checked'))
                     });
                  }
               })
          touchEvent();
   ...
   $('.in-add').click(function(){
      ...
      var todoTime = '00 : 00';
      var todoId =  todayIs();  
      var todocheck = 'check_box_outline_blank';
         ...
         function calList() {
            ...
            if(calTodos.length!=0){//메모가 있다
               ...
               if(startdate==daTe){
                  ...
                  `);
                     // 추가 해봅시다.
                     $('#calPage ul.todo').find('input.chk_'+today).click(function() {
                        $('#calPage ul.todo').find('input.chk_'+today).prop("checked",$(this).prop('checked'))
                     });
               }else{
                  if($("#calPage ul.todo").find('div').hasClass('no-todo') === false){
   ...
   $('.check>i.material-icons').on('click',function(){
      if($(this).text()=='check_box_outline_blank'){
         $(this).prev().prop('name','checked');
         $(this).empty();
         $(this).text('check_box');
         $(this).parents('li').find('.todoTitle').css({
                     'text-decoration' :'line-through',
                      color: '#ccc'
                  });
      }
      else{
         $(this).prev().prop('name','check');
         $(this).empty();
      ...
      var btnCheck=$(this).text();
      // console.log(btnCheck);
      var id=$(this).parents('.todolist-left').find('.todoId').text();
      // console.log(id);       
      var todos=getTodos();
      for (const i in todos) {
         if (todos[i].todoId==id) {
            todos[i].todocheck=btnCheck;
            break;
         }
      }
      setTodos(todos);
